...
my_location <- "cable090"
##' Location: HMS cable, 090m
##' Logger serial: 04A5 3700 374B 0F (n = 1)
##' Logger serial: 0460 1700 1840 09 (n = 1)
##' Logger serial: 0431 8200 F135 0A (n = 1)
...
d <- env_file_load(i = 2, my_serial = serial, my_files = files, my_skip = 20)
d |> 
  ggplot(aes(time, temp)) + geom_point(alpha = 0.5)
...
d |> 
  slice_head(n = 500) |> 
  filter(time > as.POSIXct("2023-04-07 12:00:00")) |> 
  ggplot(aes(time, temp)) + geom_point(alpha = 0.5)
...
#### DEPLOYMENT 3 ####
# Get file names
files <- list.files(path = "data/0431 8200 F135 0A", pattern = "\\.csv")
files
i <- 1
files[i]
...
d <- env_file_load(i = 1, my_serial = serial, my_files = files, my_skip = 20)
d |> 
  ggplot(aes(time, temp)) + geom_point(alpha = 0.5)
