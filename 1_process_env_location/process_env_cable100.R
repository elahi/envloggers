...
my_location <- "cable100"
##' Location: HMS cable, 090m
##' Logger serial: 0402 D400 A904 08 (n = 1)
...
# Get file names
files <- list.files(path = "data/0402 D400 A904 08", pattern = "\\.csv")
files
i <- 1 # first file only
...
d <- env_file_load(i = 1, my_files = files, my_skip = 20)
d |> 
  ggplot(aes(time, temp)) + geom_point(alpha = 0.5)

head(d)
d |> 
  filter(time > as.POSIXct("2021-04-14 12:00:00")) |> 
  ggplot(aes(time, temp)) + geom_point(alpha = 0.5)

tail(d)
d |> 
  slice_tail(n = 500) |> 
  filter(time < as.POSIXct("2021-06-18 09:00:00")) |> 
  ggplot(aes(time, temp)) + geom_point(alpha = 0.5)
