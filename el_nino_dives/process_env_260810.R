...
# At depth (64 minute dive)
time1 <- as_hms("11:11:00")
time2 <- as_hms("12:15:00")

# In seawater
time1 <- as_hms("11:05:00")
time2 <- as_hms("12:15:00")
...
# Dive start: 11:11 AM (REs dive computer synced with iPhone)
# Navigated to 40m on cable, placed env_logger (11) at 40m at 6', collected at 51'. 
# Dive end: 12:15 PM (64')

# DEEP
# env_logger: 11' to 25'
# start and end MLLW depths (ft): 25 and 23

# SHALLOW
# env_logger: 31' to 47'
# start and end MLLW depths (ft): 14 and 20

# Ref - Deep
time1 <- as_hms("11:24:00")
time2 <- as_hms("11:40:00")
d_deep <- d |>
  filter(time_pst > time1 & time_pst < time2)
d_deep |> 
  ggplot(aes(time_pst, temp, color = logger)) + 
...
# Ref - Shallow
time1 <- as_hms("11:46:00")
time2 <- as_hms("12:02:00")
d_shallow <- d |>
  filter(time_pst > time1 & time_pst < time2)
d_shallow |> 
  ggplot(aes(time_pst, temp, color = logger)) + 
